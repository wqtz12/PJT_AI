...
// ============================================================
// 취약점 정의 로드 (외부 JSON 파일에서 동적 로드)
// ============================================================

const VULNERABILITIES_PATH = path.join(__dirname, 'vulnerabilities.json');

/**
 * 취약점 정의를 JSON 파일에서 로드합니다.
 * 파일이 없으면 기본 취약점을 반환합니다.
 * @returns {object} 취약점 정의 객체
 */
function loadVulnerabilities() {
  try {
    if (fs.existsSync(VULNERABILITIES_PATH)) {
      const data = fs.readFileSync(VULNERABILITIES_PATH, 'utf-8');
      return JSON.parse(data);
    }
  } catch (error) {
    console.error(`취약점 파일 로드 실패: ${error.message}`);
  }
  // 파일이 없거나 로드 실패 시 빈 객체 반환 (또는 기본값)
  return {};
}

// 취약점 정의 로드
let VULNERABILITIES = loadVulnerabilities();

// 취약점 파일 변경 감지 (핫 리로드)
fs.watchFile(VULNERABILITIES_PATH, { interval: 5000 }, () => {
  console.error('취약점 파일 변경 감지, 리로드 중...');
  VULNERABILITIES = loadVulnerabilities();
  console.error(`취약점 ${Object.keys(VULNERABILITIES).length}개 로드됨`);
});
